  ...
  if (radius > 0.0) {
    ...
    for (x = [0:len(control_points)-2])
      color( (x % order == 0) ? [1.0, 0.5, 0.5] : [0.5,0.5,1.0])
        display_control_line(up23(control_points[x]),
...
module bezier_polygon(control_points,
		      order=3,
		      subdivisions=128,
                      control_point_size=0.0) {
  display_control_points1d(control_points, order, control_point_size);
  polygon(points=bezier_1d_mesh([control_points, order], subdivisions),
...
// Currently, orders up to 5 are supported.
module bezier_cylinder(control_points,
                       xorder=3,
                       yorder=3,
                       subdivisions=128,
                       control_point_size=0.0) {
  display_control_points2d(control_points, xorder, yorder, control_point_size);
  polyhedron(points=bezier_2d_mesh([control_points, xorder, yorder],
				   subdivisions),
             triangles=bezier_2d_cylinder_triangles(subdivisions));
}
...
// generate roughly subdivisions * subdivisions number of polygons.
// Currently, orders up to 5 are supported.
module bezier_torus(control_points,
		    xorder=3,
		    yorder=3,
		    subdividions=128,
		    control_point_size=0.0) {
  ...
  polyhedron(points=bezier_2d_mesh([control_points, xorder, yorder],
                                   subdivisions),
             triangles=bezier_2d_torus_triangles(subdivisions));
}
